...
import { EVENTS_PREFIX, batchTimestamp, flushEvents, listEvents, materializeEvent, spoolDepth, spoolEvents, } from './telemetry.js';
...
async function main(argv) {
    ...
    if (command === 'hooks')
        return await cmdHooks(cfg, flags.harness);
    if (command === 'daemon')
    ...
    if (command === 'emit')
        return await cmdEmit(cfg, flags);
...
async function cmdPurge(backend, cfg, olderThan, agentsOlderThan, eventsOlderThan, dryRun) {
    ...
    if (agentsOlderThan) {
        fail('registrations are never purged: presence is heartbeat-derived (idle = not on the bus), and telemetry events reference registrations. --agents-older-than was removed.');
    }
    ...
    if (!olderThan && !effectiveEvents) {
        fail('purge requires --older-than <duration> (mail archive) and/or --events <duration> (telemetry events; telemetry.retention in the config also applies), e.g. --older-than 30d (units: s, m, h, d)');
        return 1;
    }
    ...
    if (effectiveEvents) {
        ...
        const cutoff = Date.now() - maxAgeMs;
        eventVictims.push(...(await backend.list(EVENTS_PREFIX)).filter((key) => {
            const ts = batchTimestamp(key);
            return ts !== null && ts < cutoff;
        }));
    }
    if (!dryRun) {
        for (const key of [...victims, ...eventVictims])
    ...
    if (cfg.json) {
        ...
        process.stdout.write(`kept: ${keptEventBatches} event batch${keptEventBatches === 1 ? '' : 'es'}, ${keptRegistrations} registration${keptRegistrations === 1 ? '' : 's'} (registrations are never purged)\n`);
    }
    return 0;
}
...
async function cmdInit(bus, cfg, requestedHarness) {
    ...
    if (!guidanceFile) {
        throw new Error(`agentcomm: --harness must be one of ${Object.keys(HARNESS_FILE).join(', ')} (received ${harness})`);
    }
    const harnessLabel = harness === 'claude' ? 'Claude Code' : harness === 'opencode' ? 'OpenCode' : harness === 'codex' ? 'Codex' : 'AGENTS.md';
    ...
    const writeInstructions = async (filename) => {
        ...
        if (existing.includes(AGENT_INSTRUCTIONS_MARKER))
...
async function provisionOpencodeHooks() {
    const { promises: fsp } = await import('node:fs');
 ...
 */
async function autoProvisionHooks() {
    const { promises: fsp } = await import('node:fs');
    ...
    process.stderr.write(`agentcomm: OpenCode hooks were missing — wrote ${OPENCODE_HOOKS_FILE}; commit it so every session joins this bus (AGENTCOMM_NO_AUTO_HOOKS=1 disables this).\n`);
}
/**
 ...
 */
async function cmdHooks(cfg, requestedHarness) {
    const MARKETPLACE_POINTERS = {
        claude: [
    ...
    if (pointer) {
        if (cfg.json)
            emit({ harness, hooks: 'marketplace-plugin', install: pointer.slice(1).map((l) => l.trim()) });
        else
            process.stdout.write(pointer.join('\n') + '\n');
        return 0;
    }
    if (harness !== 'opencode') {
 ...
 */
async function cmdEmit(cfg, flags) {
    const { telemetry } = await loadConventions().catch(() => ({ telemetry: undefined }));
    if (!telemetry) {
        if (cfg.json)
            emit({ spooled: false, reason: 'telemetry-not-enabled' });
        else
            process.stderr.write('agentcomm: telemetry is not enabled for this repo — event dropped. Opt in with a "telemetry" section in .agentcomm.json/.yaml.\n');
        return 0;
    }
    if (!flags.type) {
    ...
    if (flags.attrs !== undefined) {
        ...
        try {
            ...
            fail(`invalid --attrs JSON: ${err.message}`);
        }
    ...
    if (flags.flush) {
        ...
        try {
            ...
            if (cfg.json)
                emit({ spooled: true, flushed: shipped, event });
            else
                process.stdout.write(`emitted ${event.type} — shipped ${shipped} event(s)\n`);
        }
        finally {
            await backend.close?.();
        }
    ...
    if (cfg.json)
        emit({ spooled: true, flushed: 0, event });
    else
        process.stdout.write(`spooled ${event.type} — ships with the next bus write (or \`emit --flush\`)\n`);
    return 0;
}
/** Read telemetry events — the analysis surface is `events --json` piped into whoever asks the question. */
async function cmdEvents(backend, cfg, flags) {
    let sinceMs;
    if (flags.since) {
    ...
    process.stdout.write(`— ${shown.length} event(s)${all.length > shown.length ? ` (of ${all.length}; raise --limit for more)` : ''}\n`);
    return 0;
}
/**
...
async function piggybackFlush(backend, cfg) {
    try {
        ...
        const shipped = await flushEvents(backend, cfg.backendUri, me);
        if (shipped > 0)
            process.stderr.write(`agentcomm: shipped ${shipped} spooled telemetry event(s) with this write\n`);
    }
    catch {
