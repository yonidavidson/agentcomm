...
export async function onTheBus(cwd) {
    if (process.env.AGENTCOMM_BACKEND)
        return true;
    let dir = cwd;
    for (;;) {
        for (const f of ['CLAUDE.md', 'AGENTS.md']) {
            try {
                if ((await fs.readFile(path.join(dir, f), 'utf8')).includes(MARKER))
                    return true;
            }
            catch {
                /* keep walking */
...
async function resolveBusUri(cwd) {
    if (process.env.AGENTCOMM_BACKEND)
        return process.env.AGENTCOMM_BACKEND;
...
export async function openBusSession(cwd) {
    try {
        if (!(await onTheBus(cwd)))
            return null;
...
export async function inboxGuardReason(s) {
    try {
        const pending = await s.bus.peek(s.alias);
        if (!pending.length)
            return null;
        const from = [...new Set(pending.map((m) => m.from))].join(', ');
        return (`agentcomm: ${pending.length} unread bus message(s) for ${s.alias} (from: ${from}). ` +
...
export async function midTurnContext(s) {
    try {
        ...
        if (!pending.length && !asks.length)
            return null;
        const lines = [];
        if (pending.length)
            lines.push(`agentcomm (mid-task): ${pending.length} unread message(s) for ${s.alias} — \`agentcomm inbox --json\` if it may affect the current work, otherwise finish first.`);
        for (const a of asks.slice(0, 2))
            lines.push(`agentcomm (mid-task): ${a.name} is asking "${a.status}" — reply only if you already know the answer; do not derail the current task.`);
        return lines.join('\n');
    }
    catch {
        return null;
