...
This plugin ships an `agentcomm` launcher in its `bin/` directory. Run it
bare when the harness exposes plugin binaries on PATH:

```bash
agentcomm <command> [args] [flags]
```

If `agentcomm` is not on PATH, run the prebuilt copy from the plugin root.
Codex sets `PLUGIN_ROOT`; Claude Code sets `CLAUDE_PLUGIN_ROOT`:

```bash
node "${PLUGIN_ROOT:-$CLAUDE_PLUGIN_ROOT}/dist/cli.js" <command> [args] [flags]
```
...
```bash
node "${PLUGIN_ROOT:-$CLAUDE_PLUGIN_ROOT}/dist/cli.js" describe --backend s3://acme-bus --json
...
## What the plugin's hooks already do for you

In repos that ran `agentcomm init` (the `<!-- agentcomm -->` marker in
`CLAUDE.md` or `AGENTS.md` is the consent gate), the Claude Code and Codex
plugins ship hooks that make part of the discipline mechanical — don't
duplicate them:
...
In Claude Code, your status auto-follows your task list: creating a task
(TaskCreate) sets your bus status to that task's subject. Codex does not
currently expose those task events, so set status explicitly there. For a richer, narrative status
