...
export async function detectRepoBus(cwd = process.cwd()): Promise<string | null> {
  ...
  if (normalized && process.env.AGENTCOMM_NO_GIT_PROBE !== '1') {
    ...
    if (cached !== null) {
      if (cached) return normalized.uri;
...
function probeCachePath(remote: string): string {
...
async function probeCacheRead(remote: string): Promise<boolean | null> {
  if (PROBE_TTL_MS <= 0) return null;
  try {
    const p = probeCachePath(remote);
    const st = await fs.stat(p);
    if (Date.now() - st.mtimeMs > PROBE_TTL_MS) return null;
...
async function probeCacheWrite(remote: string, reachable: boolean): Promise<void> {
  try {
    const p = probeCachePath(remote);
