...
registerBackend(
  ...
  (uri) => {
    if (/[?&]channel=/.test(uri)) {
      throw new Error(
        'agentcomm: file:// carves channels by path — append /<channel> to the directory instead of ?channel=.',
      );
    }
  ...
  (uri) => {
    ...
    if (q !== -1) {
      ...
      if (params.has('channel')) {
        throw new Error(
          'agentcomm: github:// carves channels by path — append /<channel> to the URI instead of ?channel=.',
        );
      }
      ...
      if (leftover.length > 0) {
        throw new Error(
          `agentcomm: unsupported query parameter(s) ${leftover.join(', ')} — only ?branch=<name> is recognized on github://.`,
        );
      }
    }
 ...
 */
function splitChannelParam(rest: string): { rest: string; channel: string } {
  const q = rest.indexOf('?');
  if (q === -1) return { rest, channel: '' };
...
function validateChannelName(name: string): void {
  if (!/^[A-Za-z0-9._-]+$/.test(name)) {
    throw new Error(
      `agentcomm: invalid channel name "${name}". Use letters, digits, '.', '_' or '-'.`,
    );
  }
}
