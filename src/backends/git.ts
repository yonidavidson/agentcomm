...
export class GitBackend implements Backend, Claimable {
  ...
  static async open(remote: string, branch = 'agentcomm', channel = ''): Promise<GitBackend> {
    const cacheRoot =
      process.env.AGENTCOMM_GIT_CACHE_DIR ??
  ...
  private git(
    args: string[],
    opts: { input?: Buffer; env?: Record<string, string>; cwd?: string; noGitDir?: boolean } = {},
  ): Promise<Buffer> {
  ...
  private async commitAndPush(
    tip: string | null,
    message: string,
    changes: { add?: { key: string; blob: string }[]; remove?: string[] },
  ): Promise<boolean> {
    ...
    try {
      if (tip) await this.git(['read-tree', tip], { env });
      else await this.git(['read-tree', '--empty'], { env });
      ...
      const commit = (
        await this.git(['commit-tree', tree, ...(tip ? ['-p', tip] : []), '-m', message])
      )
        .toString('utf8')
        .trim();
      try {
        ...
        const stderr = (err as { gitStderr?: string }).gitStderr ?? '';
        if (/non-fast-forward|fetch first|failed to push|stale info/i.test(stderr)) return false;
        throw err;
      }
    } finally {
  ...
  private async writeBlob(data: Buffer): Promise<string> {
  ...
  async delete(key: string): Promise<void> {
    for (let attempt = 1; attempt <= 6; attempt++) {
      const tip = await this.tip();
      if (tip === null) return;
      if (!(await this.exists(key, tip))) return; // absent → no-op, per the contract
      if (await this.commitAndPush(tip, `agentcomm: delete ${this.k(key)}`, { remove: [key] })) return;
      await sleep(30 * attempt + Math.floor(Math.random() * 80));
    }
    throw new Error(`agentcomm: git delete ${key} kept losing push races`);
  }

  async exists(key: string, atTip?: string): Promise<boolean> {
  ...
  async move(src: string, dst: string): Promise<void> {
    ...
    for (let attempt = 1; attempt <= 6; attempt++) {
      const tip = await this.tip();
      if (tip === null) throw notFound(src);
      let blob: string;
      try {
      ...
      if (
        await this.commitAndPush(tip, `agentcomm: move ${this.k(src)} → ${this.k(dst)} [${randomUUID().slice(0, 8)}]`, {
          add: [{ key: dst, blob }],
          remove: [src],
        })
      ) {
  ...
  async claim(queue: string, _owner: string): Promise<Message | null> {
    for (let attempt = 1; attempt <= 12; attempt++) {
      ...
      // claimed, for free).
      if (
        await this.commitAndPush(tip, `agentcomm: claim ${this.k(oldest)} by ${_owner} [${randomUUID().slice(0, 8)}]`, {
          add: [{ key: dst, blob }],
          remove: [oldest],
        })
      ) {
        return JSON.parse((await this.git(['cat-file', 'blob', blob])).toString('utf8')) as Message;
      }
