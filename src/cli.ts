...
import { LATEST_ARTIFACT_URL } from './update-check.js';
import {
  EVENTS_PREFIX,
  batchTimestamp,
  flushEvents,
...
async function cmdVersion(cfg: ResolvedConfig): Promise<number> {
  const { ownVersion, fetchLatestTag, compareVersions, LATEST_ARTIFACT_URL } = await import('./update-check.js');
  ...
  // Hardcoded on purpose: the latest artifact always lives at the same URL,
  // so scripts and automations never have to interpolate a version.
  const install = behind ? `npm install -g ${LATEST_ARTIFACT_URL}` : undefined;
  if (cfg.json) {
    emit({
  ...
  process.stdout.write(
    state === 'already-present'
      ...
      : [
          ...
          `  npm install -g ${LATEST_ARTIFACT_URL}`,
