...
export function resolveConfig(flags: ParsedFlags, env: NodeJS.ProcessEnv): ResolvedConfig {
...
export function parseArgs(argv: string[]): ParsedFlags {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    const takeVal = (): string => {
      ...
      const next = argv[++i];
      if (next === undefined) throw new Error(`agentcomm: flag --${name} expects a value`);
      return next;
    };
    switch (name) {
      case 'backend':
